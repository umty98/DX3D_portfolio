...
void Font::UpdateOpacity(string key)
{
	if (fonts.count(key) == 0)
		return;

	FontDesc* desc = fonts[key];
	desc->opacity += desc->opacityChange;
	if (desc->opacity <= 0.0f || desc->opacity >= 1.0f)
	{
		desc->opacityChange = -desc->opacityChange;
	}
